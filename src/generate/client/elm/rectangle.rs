...
This generates clientside code which decodes a joined, rectuangular table into a nested Elm record.

So, the decoders look like this:

    decodeMyPlayersAlias : Db.Read.Query MyPlayersAlias
    decodeMyPlayersAlias =
        Db.Read.query MyPlayersAlias [ Db.Read.id "myPlayersAlias__id" ]
            |> Db.Read.field "myPlayersAlias__id" Db.Read.int
            |> Db.Read.field "myPlayersAlias__aliasedName" Db.Read.string
            |> Db.Read.field "myPlayersAlias__points" Db.Read.int
            |> Db.Read.field "myPlayersAlias__status" Db.Decode.status
            |> Db.Read.nested
                (Db.Read.id "myPlayersAlias__gameId")
                (Db.Read.id "singleGame__id")
                decodeSingleGame

Though now I've made the server do the "denesting" in the db, so we use a different approach
