...
pub fn generate(
    ...
    files.push(generate_text_file(
        ...
        generate_delta_module(),
    ));
}

/// Generate the shared Db.Delta module with DeltaOp types, path parsing helpers, and lens-based generic apply functions
fn generate_delta_module() -> String {
    let mut result = String::new();

    result.push_str("module Db.Delta exposing (DeltaOp(..), decodeDeltaOp, PathSegment(..), parsePath, parsePathSegments, getAtIndex, setAtIndex, removeAtIndex, insertAtIndex, moveInList, ListLens, MaybeLens, FieldHandler, listField, maybeField, noNested, applyOps)\n\n\n");

    result.push_str("import Json.Decode as Decode\n\n\n");

    // DeltaOp type - paths are pre-parsed as (String, List PathSegment) where String is the top-level field name
    result.push_str("{-| Delta operations that can be applied to query results.\n");
    result.push_str("Paths are pre-parsed: (topLevelField, remainingSegments)\n");
    result.push_str("-}\n");
    result.push_str("type DeltaOp\n");
    result.push_str("    = SetRow ( String, List PathSegment ) Decode.Value\n");
    result.push_str("    | RemoveRow ( String, List PathSegment )\n");
    result.push_str("    | InsertRow ( String, List PathSegment ) Int Decode.Value\n");
    result.push_str("    | MoveRow ( String, List PathSegment ) Int Int\n");
    result.push_str("    | RemoveRowByIndex ( String, List PathSegment ) Int\n\n\n");

    // Path decoder helper
    result.push_str("decodePath : Decode.Decoder ( String, List PathSegment )\n");
    result.push_str("decodePath =\n");
    result.push_str("    Decode.string\n");
    result.push_str("        |> Decode.andThen\n");
    result.push_str("            (\\pathStr ->\n");
    result.push_str("                case parsePath pathStr of\n");
    result.push_str("                    Just ( field, rest ) ->\n");
    result.push_str("                        Decode.succeed ( field, parsePathSegments rest )\n\n");
    result.push_str("                    Nothing ->\n");
    result.push_str("                        Decode.fail (\"Invalid path: \" ++ pathStr)\n");
    result.push_str("            )\n\n\n");
    ...
    result.push_str("                            (Decode.field \"path\" decodePath)\n");
    ...
    // PathSegment type for nested path parsing
    result.push_str("{-| A segment in a delta path.\n");
    result.push_str("-}\n");
    result.push_str("type PathSegment\n");
    result.push_str("    = Field String\n");
    result.push_str("    | Index Int\n");
    result.push_str("    | Id String\n\n\n");

    // parsePath - parse the first segment and return remaining path
    result.push_str("{-| Parse the first field segment from a path, returning the field name and remaining path.\n");
    result.push_str("    parsePath \".user[0].posts\" == Just ( \"user\", \"[0].posts\" )\n");
    result.push_str("    parsePath \".post#(10).author\" == Just ( \"post\", \"#(10).author\" )\n");
    result.push_str("-}\n");
    ...
    result.push_str("            parseFieldName rest\n\n");
    result.push_str("        _ ->\n");
    result.push_str("            Nothing\n\n\n");

    // parseFieldName helper
    result.push_str("parseFieldName : String -> Maybe ( String, String )\n");
    result.push_str("parseFieldName str =\n");
    result.push_str("    let\n");
    result.push_str("        isFieldChar c =\n");
    result.push_str("            Char.isAlphaNum c || c == '_'\n\n");
    result.push_str("        ( fieldName, rest ) =\n");
    result.push_str("            splitWhile isFieldChar str\n");
    result.push_str("    in\n");
    result.push_str("    if String.isEmpty fieldName then\n");
    result.push_str("        Nothing\n");
    result.push_str("    else\n");
    result.push_str("        Just ( fieldName, rest )\n\n\n");

    // splitWhile helper
    result.push_str("splitWhile : (Char -> Bool) -> String -> ( String, String )\n");
    result.push_str("splitWhile pred str =\n");
    result.push_str("    splitWhileHelp pred str \"\"\n\n\n");

    result.push_str("splitWhileHelp : (Char -> Bool) -> String -> String -> ( String, String )\n");
    result.push_str("splitWhileHelp pred remaining acc =\n");
    result.push_str("    case String.uncons remaining of\n");
    result.push_str("        Just ( c, rest ) ->\n");
    result.push_str("            if pred c then\n");
    result.push_str("                splitWhileHelp pred rest (acc ++ String.fromChar c)\n");
    result.push_str("            else\n");
    result.push_str("                ( acc, remaining )\n\n");
    result.push_str("        Nothing ->\n");
    result.push_str("            ( acc, remaining )\n\n\n");

    // parsePathSegments - parse all segments from remaining path
    result.push_str("{-| Parse all path segments from a string (after the initial field).\n");
    result.push_str(
        "    parsePathSegments \"[0].posts[1]\" == [ Index 0, Field \"posts\", Index 1 ]\n",
    );
    result.push_str("    parsePathSegments \"#(10).author\" == [ Id \"10\", Field \"author\" ]\n");
    result.push_str("-}\n");
    result.push_str("parsePathSegments : String -> List PathSegment\n");
    result.push_str("parsePathSegments path =\n");
    result.push_str("    parsePathSegmentsHelp path []\n\n\n");

    result.push_str("parsePathSegmentsHelp : String -> List PathSegment -> List PathSegment\n");
    result.push_str("parsePathSegmentsHelp path acc =\n");
    result.push_str("    if String.isEmpty path then\n");
    result.push_str("        List.reverse acc\n");
    result.push_str("    else\n");
    result.push_str("        case String.uncons path of\n");
    result.push_str("            Just ( '[', rest ) ->\n");
    result.push_str("                -- Parse index: [0], [1], etc.\n");
    result.push_str("                case parseIndexSegment rest of\n");
    result.push_str("                    Just ( idx, remaining ) ->\n");
    result
        .push_str("                        parsePathSegmentsHelp remaining (Index idx :: acc)\n\n");
    result.push_str("                    Nothing ->\n");
    result.push_str("                        List.reverse acc\n\n");
    result.push_str("            Just ( '#', rest ) ->\n");
    result.push_str("                -- Parse id selector: #(10), #(user-123)\n");
    result.push_str("                case parseIdSegment rest of\n");
    result.push_str("                    Just ( id, remaining ) ->\n");
    result.push_str("                        parsePathSegmentsHelp remaining (Id id :: acc)\n\n");
    result.push_str("                    Nothing ->\n");
    result.push_str("                        List.reverse acc\n\n");
    result.push_str("            Just ( '.', rest ) ->\n");
    result.push_str("                -- Parse field name\n");
    result.push_str("                case parseFieldName rest of\n");
    result.push_str("                    Just ( fieldName, remaining ) ->\n");
    result.push_str(
    ...
    // parseIndexSegment helper
    result.push_str("parseIndexSegment : String -> Maybe ( Int, String )\n");
    result.push_str("parseIndexSegment str =\n");
    result.push_str("    let\n");
    result.push_str("        ( numStr, rest ) =\n");
    result.push_str("            splitWhile Char.isDigit str\n");
    result.push_str("    in\n");
    result.push_str("    case ( String.toInt numStr, String.uncons rest ) of\n");
    result.push_str("        ( Just idx, Just ( ']', remaining ) ) ->\n");
    result.push_str("            Just ( idx, remaining )\n\n");
    result.push_str("        _ ->\n");
    result.push_str("            Nothing\n\n\n");

    // parseIdSegment helper
    result.push_str("parseIdSegment : String -> Maybe ( String, String )\n");
    result.push_str("parseIdSegment str =\n");
    result.push_str("    case String.uncons str of\n");
    result.push_str("        Just ( '(', rest ) ->\n");
    result.push_str("            parseIdContent rest \"\"\n\n");
    result.push_str("        _ ->\n");
    result.push_str("            Nothing\n\n\n");

    result.push_str("parseIdContent : String -> String -> Maybe ( String, String )\n");
    result.push_str("parseIdContent str acc =\n");
    result.push_str("    case String.uncons str of\n");
    result.push_str("        Just ( ')', rest ) ->\n");
    result.push_str("            Just ( acc, rest )\n\n");
    result.push_str("        Just ( '\\\\', rest ) ->\n");
    result.push_str("            -- Handle escaping\n");
    result.push_str("            case String.uncons rest of\n");
    result.push_str("                Just ( c, remaining ) ->\n");
    result.push_str("                    parseIdContent remaining (acc ++ String.fromChar c)\n\n");
    result.push_str("                Nothing ->\n");
    result.push_str("                    Nothing\n\n");
    result.push_str("        Just ( c, rest ) ->\n");
    result.push_str("            parseIdContent rest (acc ++ String.fromChar c)\n\n");
    result.push_str("        Nothing ->\n");
    result.push_str("            Nothing\n\n\n");

    // applyAtPath - generic function to apply an operation at a nested path
    result.push_str(
        "{-| Apply a function at a specific path within a list, handling nested structures.\n",
    );
    result.push_str("    The function receives the remaining path segments and the item at the current index.\n");
    result.push_str("-}\n");
    result.push_str("applyAtPath : List PathSegment -> (List PathSegment -> a -> Result String a) -> List a -> Result String (List a)\n");
    result.push_str("applyAtPath segments fn list =\n");
    result.push_str("    case segments of\n");
    result.push_str("        [] ->\n");
    result.push_str("            -- No more segments, this shouldn't happen in normal use\n");
    result.push_str("            Ok list\n\n");
    result.push_str("        (Index idx) :: rest ->\n");
    result.push_str("            case getAtIndex idx list of\n");
    result.push_str("                Just item ->\n");
    result.push_str("                    case fn rest item of\n");
    result.push_str("                        Ok newItem ->\n");
    result.push_str("                            Ok (setAtIndex idx newItem list)\n\n");
    result.push_str("                        Err err ->\n");
    result.push_str("                            Err err\n\n");
    result.push_str("                Nothing ->\n");
    result
        .push_str("                    Err (\"Index out of bounds: \" ++ String.fromInt idx)\n\n");
    result.push_str("        (Id _) :: _ ->\n");
    result
        .push_str("            -- Id lookups need to be handled by the specific type's applier\n");
    result.push_str("            Err \"Id selectors must be handled by type-specific code\"\n\n");
    result.push_str("        (Field _) :: _ ->\n");
    result.push_str("            -- Field access at list level doesn't make sense\n");
    result.push_str("            Err \"Cannot access field on a list\"\n\n\n");

    // List manipulation helpers
    result.push_str("{-| Get an item at a specific index in a list.\n");
    result.push_str("-}\n");
    result.push_str("getAtIndex : Int -> List a -> Maybe a\n");
    result.push_str("getAtIndex idx list =\n");
    result.push_str("    List.drop idx list |> List.head\n\n\n");

    result.push_str("{-| Set an item at a specific index in a list.\n");
    result.push_str("-}\n");
    result.push_str("setAtIndex : Int -> a -> List a -> List a\n");
    result.push_str("setAtIndex idx item list =\n");
    result.push_str("    List.indexedMap\n");
    result.push_str("        (\\i existing ->\n");
    result.push_str("            if i == idx then\n");
    ...
    result.push_str("{-| Remove an item at a specific index from a list.\n");
    result.push_str("-}\n");
    result.push_str("removeAtIndex : Int -> List a -> List a\n");
    result.push_str("removeAtIndex idx list =\n");
    result.push_str("    List.take idx list ++ List.drop (idx + 1) list\n\n\n");

    result.push_str("{-| Insert an item at a specific index in a list.\n");
    result.push_str("-}\n");
    result.push_str("insertAtIndex : Int -> a -> List a -> List a\n");
    result.push_str("insertAtIndex idx item list =\n");
    result.push_str("    List.take idx list ++ [ item ] ++ List.drop idx list\n\n\n");

    result.push_str("{-| Move an item from one index to another in a list.\n");
    result.push_str("-}\n");
    result.push_str("moveInList : Int -> Int -> List a -> List a\n");
    result.push_str("moveInList from to list =\n");
    result.push_str("    case getAtIndex from list of\n");
    result.push_str("        Just item ->\n");
    result.push_str("            let\n");
    result.push_str("                without =\n");
    result.push_str("                    removeAtIndex from list\n");
    result.push_str("            in\n");
    result.push_str("            insertAtIndex to item without\n\n");
    result.push_str("        Nothing ->\n");
    result.push_str("            list\n\n\n");

    // Add lens types and generic apply functions
    result.push_str(&generate_lens_types_and_apply_functions());

    result
}

/// Generate lens types and generic apply functions for the Delta module
fn generate_lens_types_and_apply_functions() -> String {
    let mut result = String::new();

    // LENS TYPES
    result.push_str("-- LENS TYPES\n\n\n");

    result.push_str("{-| A lens for accessing a List field on a record.\n");
    result.push_str("-}\n");
    result.push_str("type alias ListLens record item =\n");
    result.push_str("    { get : record -> List item\n");
    result.push_str("    , set : List item -> record -> record\n");
    result.push_str("    , decode : Decode.Decoder item\n");
    result.push_str("    , nested : String -> Maybe (FieldHandler item)\n");
    result.push_str("    }\n\n\n");

    result.push_str("{-| A lens for accessing a Maybe field on a record.\n");
    result.push_str("-}\n");
    result.push_str("type alias MaybeLens record item =\n");
    result.push_str("    { get : record -> Maybe item\n");
    result.push_str("    , set : Maybe item -> record -> record\n");
    result.push_str("    , decode : Decode.Decoder item\n");
    result.push_str("    , nested : String -> Maybe (FieldHandler item)\n");
    result.push_str("    }\n\n\n");

    result.push_str("{-| A field handler encapsulates all delta operations for a single field.\n");
    result.push_str("This allows heterogeneous field types to be handled uniformly.\n");
    result.push_str("-}\n");
    result.push_str("type FieldHandler record\n");
    result.push_str("    = ListHandler (ListFieldHandler record)\n");
    result.push_str("    | MaybeHandler (MaybeFieldHandler record)\n\n\n");

    result.push_str("type alias ListFieldHandler record =\n");
    result.push_str(
        "    { setRow : List PathSegment -> Decode.Value -> record -> Result String record\n",
    );
    result.push_str("    , removeRow : List PathSegment -> record -> Result String record\n");
    result.push_str("    , insertRow : List PathSegment -> Int -> Decode.Value -> record -> Result String record\n");
    result.push_str(
    ...
    result.push_str("type alias MaybeFieldHandler record =\n");
    result.push_str(
        "    { setRow : List PathSegment -> Decode.Value -> record -> Result String record\n",
    );
    result.push_str("    , removeRow : List PathSegment -> record -> Result String record\n");
    ...
    // LENS CONSTRUCTORS
    result.push_str("-- LENS CONSTRUCTORS\n\n\n");

    result.push_str("{-| Create a FieldHandler for a List field.\n");
    result.push_str("-}\n");
    result.push_str("listField : ListLens record item -> FieldHandler record\n");
    result.push_str("listField lens =\n");
    result.push_str("    ListHandler\n");
    result.push_str("        { setRow = setRowInList lens\n");
    result.push_str("        , removeRow = removeRowFromList lens\n");
    result.push_str("        , insertRow = insertRowInList lens\n");
    result.push_str("        , moveRow = moveRowInListField lens\n");
    result.push_str("        , removeByIndex = removeByIndexFromList lens\n");
    result.push_str("        }\n\n\n");

    result.push_str("{-| Create a FieldHandler for a Maybe field.\n");
    result.push_str("-}\n");
    result.push_str("maybeField : MaybeLens record item -> FieldHandler record\n");
    result.push_str("maybeField lens =\n");
    result.push_str("    MaybeHandler\n");
    result.push_str("        { setRow = setRowInMaybe lens\n");
    result.push_str("        , removeRow = removeRowFromMaybe lens\n");
    result.push_str("        }\n\n\n");

    result.push_str("{-| Helper for fields with no nested children.\n");
    result.push_str("-}\n");
    result.push_str("noNested : String -> Maybe (FieldHandler item)\n");
    result.push_str("noNested _ =\n");
    result.push_str("    Nothing\n\n\n");

    // LIST FIELD OPERATIONS
    result.push_str("-- LIST FIELD OPERATIONS\n\n\n");

    // setRowInList
    result.push_str("setRowInList : ListLens record item -> List PathSegment -> Decode.Value -> record -> Result String record\n");
    result.push_str("setRowInList lens segments json record =\n");
    result.push_str("    case segments of\n");
    result.push_str("        [] ->\n");
    result.push_str("            Err \"Cannot set row without index\"\n\n");
    result.push_str("        (Index idx) :: rest ->\n");
    result.push_str("            let\n");
    result.push_str("                list =\n");
    result.push_str("                    lens.get record\n");
    result.push_str("            in\n");
    result.push_str("            case getAtIndex idx list of\n");
    result.push_str("                Just item ->\n");
    result.push_str("                    if List.isEmpty rest then\n");
    result.push_str("                        case Decode.decodeValue lens.decode json of\n");
    result.push_str("                            Ok row ->\n");
    result.push_str(
        "                                Ok (lens.set (setAtIndex idx row list) record)\n\n",
    );
    result.push_str("                            Err err ->\n");
    result.push_str("                                Err (\"Failed to decode row: \" ++ Decode.errorToString err)\n\n");
    result.push_str("                    else\n");
    result.push_str("                        applyNestedSetRow lens.nested rest json item\n");
    result.push_str("                            |> Result.map (\\newItem -> lens.set (setAtIndex idx newItem list) record)\n\n");
    result.push_str("                Nothing ->\n");
    result
        .push_str("                    Err (\"Index out of bounds: \" ++ String.fromInt idx)\n\n");
    result.push_str("        (Id id) :: rest ->\n");
    result.push_str("            let\n");
    result.push_str("                list =\n");
    result.push_str("                    lens.get record\n");
    result.push_str("            in\n");
    result.push_str("            case findByIdWithIndex id list of\n");
    result.push_str("                Just ( idx, item ) ->\n");
    result.push_str("                    if List.isEmpty rest then\n");
    result.push_str("                        case Decode.decodeValue lens.decode json of\n");
    result.push_str("                            Ok row ->\n");
    result.push_str(
    ...
    result.push_str("                            |> Result.map (\\newItem -> lens.set (setAtIndex idx newItem list) record)\n\n");
    result.push_str("                Nothing ->\n");
    result.push_str("                    Err (\"Id not found: \" ++ id)\n\n");
    result.push_str("        (Field _) :: _ ->\n");
    result.push_str("            Err \"Unexpected field segment at list level\"\n\n\n");

    // removeRowFromList
    result.push_str("removeRowFromList : ListLens record item -> List PathSegment -> record -> Result String record\n");
    result.push_str("removeRowFromList lens segments record =\n");
    result.push_str("    case segments of\n");
    result.push_str("        [] ->\n");
    result.push_str("            Err \"Cannot remove row without index\"\n\n");
    result.push_str("        [ Index idx ] ->\n");
    result.push_str("            Ok (lens.set (removeAtIndex idx (lens.get record)) record)\n\n");
    result.push_str("        [ Id id ] ->\n");
    result.push_str("            case findByIdWithIndex id (lens.get record) of\n");
    result.push_str("                Just ( idx, _ ) ->\n");
    result.push_str(
    ...
    result.push_str("            case getAtIndex idx list of\n");
    result.push_str("                Just item ->\n");
    result.push_str("                    applyNestedRemoveRow lens.nested rest item\n");
    result.push_str("                        |> Result.map (\\newItem -> lens.set (setAtIndex idx newItem list) record)\n\n");
    result.push_str("                Nothing ->\n");
    result
        .push_str("                    Err (\"Index out of bounds: \" ++ String.fromInt idx)\n\n");
    ...
    // insertRowInList
    result.push_str("insertRowInList : ListLens record item -> List PathSegment -> Int -> Decode.Value -> record -> Result String record\n");
    result.push_str("insertRowInList lens segments index json record =\n");
    result.push_str("    case segments of\n");
    result.push_str("        [] ->\n");
    result.push_str("            -- Insert at this list\n");
    result.push_str("            case Decode.decodeValue lens.decode json of\n");
    result.push_str("                Ok row ->\n");
    result.push_str(
    ...
    result.push_str("            case getAtIndex idx list of\n");
    result.push_str("                Just item ->\n");
    result.push_str("                    applyNestedInsertRow lens.nested rest index json item\n");
    result.push_str("                        |> Result.map (\\newItem -> lens.set (setAtIndex idx newItem list) record)\n\n");
    result.push_str("                Nothing ->\n");
    result
        .push_str("                    Err (\"Index out of bounds: \" ++ String.fromInt idx)\n\n");
    ...
    // moveRowInListField
    result.push_str("moveRowInListField : ListLens record item -> List PathSegment -> Int -> Int -> record -> Result String record\n");
    result.push_str("moveRowInListField lens segments from to record =\n");
    result.push_str("    case segments of\n");
    result.push_str("        [] ->\n");
    result.push_str("            -- Move at this list level\n");
    result.push_str("            Ok (lens.set (moveInList from to (lens.get record)) record)\n\n");
    result.push_str("        (Index idx) :: rest ->\n");
    result.push_str("            let\n");
    result.push_str("                list =\n");
    result.push_str("                    lens.get record\n");
    ...
    result.push_str("            case getAtIndex idx list of\n");
    result.push_str("                Just item ->\n");
    result.push_str("                    applyNestedMoveRow lens.nested rest from to item\n");
    result.push_str("                        |> Result.map (\\newItem -> lens.set (setAtIndex idx newItem list) record)\n\n");
    result.push_str("                Nothing ->\n");
    result
        .push_str("                    Err (\"Index out of bounds: \" ++ String.fromInt idx)\n\n");
    ...
    // removeByIndexFromList
    result.push_str("removeByIndexFromList : ListLens record item -> List PathSegment -> Int -> record -> Result String record\n");
    result.push_str("removeByIndexFromList lens segments index record =\n");
    result.push_str("    case segments of\n");
    result.push_str("        [] ->\n");
    result.push_str("            -- Remove at this list level\n");
    result.push_str("            Ok (lens.set (removeAtIndex index (lens.get record)) record)\n\n");
    result.push_str("        (Index idx) :: rest ->\n");
    result.push_str("            let\n");
    result.push_str("                list =\n");
    result.push_str("                    lens.get record\n");
    ...
    result.push_str("            case getAtIndex idx list of\n");
    result.push_str("                Just item ->\n");
    result.push_str("                    applyNestedRemoveByIndex lens.nested rest index item\n");
    result.push_str("                        |> Result.map (\\newItem -> lens.set (setAtIndex idx newItem list) record)\n\n");
    result.push_str("                Nothing ->\n");
    result
        .push_str("                    Err (\"Index out of bounds: \" ++ String.fromInt idx)\n\n");
    ...
    result.push_str("-- MAYBE FIELD OPERATIONS\n\n\n");

    // setRowInMaybe
    result.push_str("setRowInMaybe : MaybeLens record item -> List PathSegment -> Decode.Value -> record -> Result String record\n");
    result.push_str("setRowInMaybe lens segments json record =\n");
    result.push_str("    case segments of\n");
    result.push_str("        [] ->\n");
    result.push_str("            -- Set the Maybe value directly\n");
    result.push_str("            case Decode.decodeValue lens.decode json of\n");
    result.push_str("                Ok row ->\n");
    result.push_str("                    Ok (lens.set (Just row) record)\n\n");
    result.push_str("                Err err ->\n");
    result.push_str(
        "                    Err (\"Failed to decode row: \" ++ Decode.errorToString err)\n\n",
    );
    result.push_str("        _ ->\n");
    result.push_str("            -- Navigate into the Maybe value\n");
    result.push_str("            case lens.get record of\n");
    result.push_str("                Just item ->\n");
    result.push_str("                    applyNestedSetRow lens.nested segments json item\n");
    result.push_str(
        "                        |> Result.map (\\newItem -> lens.set (Just newItem) record)\n\n",
    );
    result.push_str("                Nothing ->\n");
    result.push_str("                    Err \"Cannot navigate into Nothing\"\n\n\n");

    // removeRowFromMaybe
    result.push_str("removeRowFromMaybe : MaybeLens record item -> List PathSegment -> record -> Result String record\n");
    result.push_str("removeRowFromMaybe lens segments record =\n");
    result.push_str("    case segments of\n");
    result.push_str("        [] ->\n");
    result.push_str("            -- Remove the Maybe value (set to Nothing)\n");
    result.push_str("            Ok (lens.set Nothing record)\n\n");
    result.push_str("        _ ->\n");
    result.push_str("            -- Navigate into the Maybe value\n");
    result.push_str("            case lens.get record of\n");
    result.push_str("                Just item ->\n");
    result.push_str("                    applyNestedRemoveRow lens.nested segments item\n");
    result.push_str(
        "                        |> Result.map (\\newItem -> lens.set (Just newItem) record)\n\n",
    );
    result.push_str("                Nothing ->\n");
    ...
    result.push_str("-- NESTED FIELD DISPATCH\n\n\n");

    // applyNestedSetRow
    result.push_str("applyNestedSetRow : (String -> Maybe (FieldHandler item)) -> List PathSegment -> Decode.Value -> item -> Result String item\n");
    result.push_str("applyNestedSetRow nestedLookup segments json item =\n");
    result.push_str("    case segments of\n");
    result.push_str("        (Field fieldName) :: rest ->\n");
    result.push_str("            case nestedLookup fieldName of\n");
    result.push_str("                Just (ListHandler handler) ->\n");
    result.push_str("                    handler.setRow rest json item\n\n");
    result.push_str("                Just (MaybeHandler handler) ->\n");
    result.push_str("                    handler.setRow rest json item\n\n");
    result.push_str("                Nothing ->\n");
    result.push_str("                    Err (\"Unknown nested field: \" ++ fieldName)\n\n");
    result.push_str("        _ ->\n");
    result.push_str("            Err \"Expected field segment for nested access\"\n\n\n");

    // applyNestedRemoveRow
    result.push_str("applyNestedRemoveRow : (String -> Maybe (FieldHandler item)) -> List PathSegment -> item -> Result String item\n");
    result.push_str("applyNestedRemoveRow nestedLookup segments item =\n");
    result.push_str("    case segments of\n");
    result.push_str("        (Field fieldName) :: rest ->\n");
    result.push_str("            case nestedLookup fieldName of\n");
    result.push_str("                Just (ListHandler handler) ->\n");
    result.push_str("                    handler.removeRow rest item\n\n");
    result.push_str("                Just (MaybeHandler handler) ->\n");
    result.push_str("                    handler.removeRow rest item\n\n");
    result.push_str("                Nothing ->\n");
    result.push_str("                    Err (\"Unknown nested field: \" ++ fieldName)\n\n");
    ...
    // applyNestedInsertRow
    result.push_str("applyNestedInsertRow : (String -> Maybe (FieldHandler item)) -> List PathSegment -> Int -> Decode.Value -> item -> Result String item\n");
    result.push_str("applyNestedInsertRow nestedLookup segments index json item =\n");
    result.push_str("    case segments of\n");
    result.push_str("        (Field fieldName) :: rest ->\n");
    result.push_str("            case nestedLookup fieldName of\n");
    result.push_str("                Just (ListHandler handler) ->\n");
    result.push_str("                    handler.insertRow rest index json item\n\n");
    result.push_str("                Just (MaybeHandler _) ->\n");
    result.push_str("                    Err \"Cannot insert into Maybe field\"\n\n");
    result.push_str("                Nothing ->\n");
    result.push_str("                    Err (\"Unknown nested field: \" ++ fieldName)\n\n");
    result.push_str("        _ ->\n");
    result.push_str("            Err \"Expected field segment for nested access\"\n\n\n");
    ...
    // applyNestedMoveRow
    result.push_str("applyNestedMoveRow : (String -> Maybe (FieldHandler item)) -> List PathSegment -> Int -> Int -> item -> Result String item\n");
    result.push_str("applyNestedMoveRow nestedLookup segments from to item =\n");
    result.push_str("    case segments of\n");
    result.push_str("        (Field fieldName) :: rest ->\n");
    result.push_str("            case nestedLookup fieldName of\n");
    result.push_str("                Just (ListHandler handler) ->\n");
    result.push_str("                    handler.moveRow rest from to item\n\n");
    result.push_str("                Just (MaybeHandler _) ->\n");
    result.push_str("                    Err \"Cannot move in Maybe field\"\n\n");
    result.push_str("                Nothing ->\n");
    result.push_str("                    Err (\"Unknown nested field: \" ++ fieldName)\n\n");
    result.push_str("        _ ->\n");
    result.push_str("            Err \"Expected field segment for nested access\"\n\n\n");

    // applyNestedRemoveByIndex
    result.push_str("applyNestedRemoveByIndex : (String -> Maybe (FieldHandler item)) -> List PathSegment -> Int -> item -> Result String item\n");
    result.push_str("applyNestedRemoveByIndex nestedLookup segments index item =\n");
    result.push_str("    case segments of\n");
    result.push_str("        (Field fieldName) :: rest ->\n");
    result.push_str("            case nestedLookup fieldName of\n");
    result.push_str("                Just (ListHandler handler) ->\n");
    result.push_str("                    handler.removeByIndex rest index item\n\n");
    result.push_str("                Just (MaybeHandler _) ->\n");
    result.push_str("                    Err \"Cannot remove by index from Maybe field\"\n\n");
    result.push_str("                Nothing ->\n");
    result.push_str("                    Err (\"Unknown nested field: \" ++ fieldName)\n\n");
    result.push_str("        _ ->\n");
    result.push_str("            Err \"Expected field segment for nested access\"\n\n\n");
    ...
    // ID LOOKUP HELPER
    result.push_str("-- ID LOOKUP HELPER\n\n\n");

    result.push_str("{-| Find an item by id in a list. Returns the index and item.\n");
    result
        .push_str("Note: This is a placeholder - actual id lookup requires type-specific code.\n");
    result.push_str("-}\n");
    result.push_str("findByIdWithIndex : String -> List a -> Maybe ( Int, a )\n");
    result.push_str("findByIdWithIndex targetId list =\n");
    result.push_str("    findByIdWithIndexHelp targetId 0 list\n\n\n");

    result.push_str("findByIdWithIndexHelp : String -> Int -> List a -> Maybe ( Int, a )\n");
    result.push_str("findByIdWithIndexHelp targetId idx list =\n");
    result.push_str("    case list of\n");
    result.push_str("        [] ->\n");
    result.push_str("            Nothing\n\n");
    result.push_str("        _ :: rest ->\n");
    result.push_str("            -- Cannot generically access .id in Elm, so id selectors\n");
    result.push_str("            -- need Index paths from the server instead\n");
    result.push_str("            findByIdWithIndexHelp targetId (idx + 1) rest\n\n\n");

    // GENERIC APPLY
    result.push_str("-- GENERIC APPLY\n\n\n");

    result.push_str("{-| Apply a list of delta operations to a record using field handlers.\n");
    result.push_str("-}\n");
    result.push_str("applyOps : List ( String, FieldHandler record ) -> List DeltaOp -> record -> Result String record\n");
    result.push_str("applyOps fields ops data =\n");
    result.push_str("    List.foldl\n");
    result.push_str("        (\\op acc ->\n");
    result.push_str("            Result.andThen (applyOp fields op) acc\n");
    result.push_str("        )\n");
    result.push_str("        (Ok data)\n");
    result.push_str("        ops\n\n\n");

    result.push_str("applyOp : List ( String, FieldHandler record ) -> DeltaOp -> record -> Result String record\n");
    result.push_str("applyOp fields op record =\n");
    result.push_str("    let\n");
    result.push_str("        lookupField name =\n");
    result.push_str("            List.filterMap\n");
    result.push_str("                (\\( n, handler ) ->\n");
    result.push_str("                    if n == name then\n");
    result.push_str("                        Just handler\n\n");
    result.push_str("                    else\n");
    result.push_str("                        Nothing\n");
    result.push_str("                )\n");
    result.push_str("                fields\n");
    result.push_str("                |> List.head\n");
    result.push_str("    in\n");
    result.push_str("    case op of\n");
    result.push_str("        SetRow ( field, segments ) json ->\n");
    result.push_str("            case lookupField field of\n");
    result.push_str("                Just (ListHandler handler) ->\n");
    result.push_str("                    handler.setRow segments json record\n\n");
    result.push_str("                Just (MaybeHandler handler) ->\n");
    result.push_str("                    handler.setRow segments json record\n\n");
    result.push_str("                Nothing ->\n");
    result.push_str("                    Err (\"Unknown field: \" ++ field)\n\n");
    result.push_str("        RemoveRow ( field, segments ) ->\n");
    result.push_str("            case lookupField field of\n");
    result.push_str("                Just (ListHandler handler) ->\n");
    result.push_str("                    handler.removeRow segments record\n\n");
    result.push_str("                Just (MaybeHandler handler) ->\n");
    result.push_str("                    handler.removeRow segments record\n\n");
    result.push_str("                Nothing ->\n");
    result.push_str("                    Err (\"Unknown field: \" ++ field)\n\n");
    result.push_str("        InsertRow ( field, segments ) index json ->\n");
    result.push_str("            case lookupField field of\n");
    result.push_str("                Just (ListHandler handler) ->\n");
    result.push_str("                    handler.insertRow segments index json record\n\n");
    result.push_str("                Just (MaybeHandler _) ->\n");
    result.push_str("                    Err \"Cannot insert into Maybe field\"\n\n");
    result.push_str("                Nothing ->\n");
    result.push_str("                    Err (\"Unknown field: \" ++ field)\n\n");
    result.push_str("        MoveRow ( field, segments ) from to ->\n");
    result.push_str("            case lookupField field of\n");
    result.push_str("                Just (ListHandler handler) ->\n");
    result.push_str("                    handler.moveRow segments from to record\n\n");
    result.push_str("                Just (MaybeHandler _) ->\n");
    result.push_str("                    Err \"Cannot move in Maybe field\"\n\n");
    result.push_str("                Nothing ->\n");
    result.push_str("                    Err (\"Unknown field: \" ++ field)\n\n");
    result.push_str("        RemoveRowByIndex ( field, segments ) index ->\n");
    result.push_str("            case lookupField field of\n");
    result.push_str("                Just (ListHandler handler) ->\n");
    result.push_str("                    handler.removeByIndex segments index record\n\n");
    result.push_str("                Just (MaybeHandler _) ->\n");
    result.push_str("                    Err \"Cannot remove by index from Maybe field\"\n\n");
    result.push_str("                Nothing ->\n");
    result.push_str("                    Err (\"Unknown field: \" ++ field)\n");
...
// Legacy functions below - kept for reference but no longer used

fn to_apply_set_row_function(query: &ast::Query) -> String {
    let mut result = String::new();

    result.push_str(
        "applySetRow : ( String, List Db.Delta.PathSegment ) -> Decode.Value -> ReturnData -> Result String ReturnData\n",
    );
    result.push_str("applySetRow ( field, segments ) rowJson data =\n");
    result.push_str("    case field of\n");
    ...
    for field in &query.fields {
        if let ast::TopLevelQueryField::Field(query_field) = field {
            ...
            result.push_str(&format!("        \"{}\" ->\n", field_name));
            result.push_str(&format!(
                "            applySetRowTo{} segments rowJson data.{}\n",
                capitalized, field_name
            ));
            result.push_str(&format!(
                "                |> Result.map (\\newList -> {{ data | {} = newList }})\n\n",
                field_name
    ...
    // Generate type-specific setRow functions for each top-level field
    for field in &query.fields {
        if let ast::TopLevelQueryField::Field(query_field) = field {
            result.push_str(&generate_set_row_type_function(query_field));
        }
    }
...
/// Generate the applySetRowTo{Type} and applySetRowNested{Type} functions for a query field
fn generate_set_row_type_function(query_field: &ast::QueryField) -> String {
    let mut result = String::new();
    let field_name = ast::get_aliased_name(query_field);
    let capitalized = string::capitalize(&field_name);

    result.push_str(&format!(
        "applySetRowTo{} : List Db.Delta.PathSegment -> Decode.Value -> List {} -> Result String (List {})\n",
        capitalized, capitalized, capitalized
    ));
    result.push_str(&format!(
        "applySetRowTo{} segments rowJson list =\n",
        capitalized
    ));
    result.push_str("    case segments of\n");
    result.push_str("        [] ->\n");
    result.push_str("            Err \"Cannot set row without index\"\n\n");
    result.push_str("        (Db.Delta.Index idx) :: restSegments ->\n");
    result.push_str("            case Db.Delta.getAtIndex idx list of\n");
    result.push_str("                Just item ->\n");
    result.push_str("                    if List.isEmpty restSegments then\n");
    result.push_str(&format!(
        "                        case Decode.decodeValue decode{} rowJson of\n",
        capitalized
    ));
    result.push_str("                            Ok row ->\n");
    result.push_str("                                Ok (Db.Delta.setAtIndex idx row list)\n\n");
    result.push_str("                            Err err ->\n");
    result.push_str("                                Err (\"Failed to decode row: \" ++ Decode.errorToString err)\n\n");
    result.push_str("                    else\n");
    result.push_str(&format!(
        "                        applySetRowNested{} restSegments rowJson item\n",
        capitalized
    ));
    result.push_str("                            |> Result.map (\\newItem -> Db.Delta.setAtIndex idx newItem list)\n\n");
    result.push_str("                Nothing ->\n");
    result
        .push_str("                    Err (\"Index out of bounds: \" ++ String.fromInt idx)\n\n");
    result.push_str("        _ ->\n");
    result.push_str("            Err \"Invalid path segment for list\"\n\n\n");

    // Generate nested update function
    result.push_str(&format!(
        "applySetRowNested{} : List Db.Delta.PathSegment -> Decode.Value -> {} -> Result String {}\n",
        capitalized, capitalized, capitalized
    ));
    result.push_str(&format!(
        "applySetRowNested{} segments rowJson item =\n",
        capitalized
    ));
    result.push_str("    case segments of\n");

    // Find nested list fields in this query field
    let nested_fields = collect_nested_list_fields(query_field);

    if nested_fields.is_empty() {
        result.push_str("        _ ->\n");
        result.push_str("            Err \"No nested fields to update\"\n\n\n");
    } else {
        for (nested_name, nested_type) in &nested_fields {
            result.push_str(&format!(
                "        (Db.Delta.Field \"{}\") :: restSegments ->\n",
                nested_name
            ));
            result.push_str(&format!(
                "            applySetRowTo{} restSegments rowJson item.{}\n",
                nested_type, nested_name
            ));
            result.push_str(&format!(
                "                |> Result.map (\\newNested -> {{ item | {} = newNested }})\n\n",
                nested_name
            ));
        }
        result.push_str("        _ ->\n");
        result.push_str("            Err \"Unknown nested field\"\n\n\n");
    }

    // Generate nested type functions recursively
    for (nested_name, nested_type) in &nested_fields {
        if let Some(nested_field) = find_nested_query_field(query_field, nested_name) {
            result.push_str(&generate_set_row_nested_type_function(
                nested_field,
                nested_type,
            ));
        }
...
/// Generate setRow functions for nested types
fn generate_set_row_nested_type_function(query_field: &ast::QueryField, type_name: &str) -> String {
    let mut result = String::new();
    ...
    // Generate nested update function for this type
    let nested_fields = collect_nested_list_fields(query_field);
...
/// Collect nested list fields from a query field (returns field name and type name pairs)
fn collect_nested_list_fields(query_field: &ast::QueryField) -> Vec<(String, String)> {
    let mut nested = Vec::new();
    let parent_name = ast::get_aliased_name(query_field);
    let parent_capitalized = string::capitalize(&parent_name);
    ...
    for arg_field in &query_field.fields {
        if let ast::ArgField::Field(nested_field) = arg_field {
            // Check if this nested field has its own fields (making it a relationship/list)
            let has_fields = nested_field
                .fields
                .iter()
                .any(|f| matches!(f, ast::ArgField::Field(_)));
            if has_fields {
                ...
                let nested_type =
                    format!("{}_{}", parent_capitalized, string::capitalize(nested_name));
                nested.push((nested_name.clone(), nested_type));
            }
        }
    }
    nested
...
fn to_apply_remove_row_function(query: &ast::Query) -> String {
    ...
    result.push_str("applyRemoveRow : ( String, List Db.Delta.PathSegment ) -> ReturnData -> Result String ReturnData\n");
    result.push_str("applyRemoveRow ( field, segments ) data =\n");
    result.push_str("    case field of\n");
    ...
    for field in &query.fields {
        if let ast::TopLevelQueryField::Field(query_field) = field {
            ...
            result.push_str(&format!("        \"{}\" ->\n", field_name));
            result.push_str(&format!(
                "            applyRemoveRowFrom{} segments data.{}\n",
                capitalized, field_name
            ));
            result.push_str(&format!(
                "                |> Result.map (\\newList -> {{ data | {} = newList }})\n\n",
    ...
    // Generate type-specific removeRow functions
    for field in &query.fields {
        if let ast::TopLevelQueryField::Field(query_field) = field {
            result.push_str(&generate_remove_row_type_function(query_field));
        }
    }
...
/// Generate the applyRemoveRowFrom{Type} function for a query field
fn generate_remove_row_type_function(query_field: &ast::QueryField) -> String {
    let mut result = String::new();
    let field_name = ast::get_aliased_name(query_field);
    let capitalized = string::capitalize(&field_name);

    result.push_str(&format!(
        "applyRemoveRowFrom{} : List Db.Delta.PathSegment -> List {} -> Result String (List {})\n",
        capitalized, capitalized, capitalized
    ));
    result.push_str(&format!(
        "applyRemoveRowFrom{} segments list =\n",
        capitalized
    ));
    result.push_str("    case segments of\n");
    result.push_str("        [] ->\n");
    result.push_str("            Err \"Cannot remove row without index\"\n\n");
    result.push_str("        [ Db.Delta.Index idx ] ->\n");
    result.push_str("            Ok (Db.Delta.removeAtIndex idx list)\n\n");
    result.push_str("        (Db.Delta.Index idx) :: restSegments ->\n");
    result.push_str("            case Db.Delta.getAtIndex idx list of\n");
    result.push_str("                Just item ->\n");
    result.push_str(&format!(
        "                    applyRemoveRowNested{} restSegments item\n",
        capitalized
    ));
    result.push_str("                        |> Result.map (\\newItem -> Db.Delta.setAtIndex idx newItem list)\n\n");
    result.push_str("                Nothing ->\n");
    ...
    // Generate nested remove function
    let nested_fields = collect_nested_list_fields(query_field);

    result.push_str(&format!(
        "applyRemoveRowNested{} : List Db.Delta.PathSegment -> {} -> Result String {}\n",
        capitalized, capitalized, capitalized
    ));
    result.push_str(&format!(
        "applyRemoveRowNested{} segments item =\n",
        capitalized
    ));
    result.push_str("    case segments of\n");
    ...
    if nested_fields.is_empty() {
        ...
        for (nested_name, nested_type) in &nested_fields {
            ...
            result.push_str(&format!(
                "            applyRemoveRowFrom{} restSegments item.{}\n",
                nested_type, nested_name
            ));
            result.push_str(&format!(
                "                |> Result.map (\\newNested -> {{ item | {} = newNested }})\n\n",
    ...
    for (nested_name, nested_type) in &nested_fields {
        if let Some(nested_field) = find_nested_query_field(query_field, nested_name) {
            result.push_str(&generate_remove_row_nested_type_function(
                nested_field,
                nested_type,
            ));
        }
...
/// Generate removeRow functions for nested types
fn generate_remove_row_nested_type_function(
    query_field: &ast::QueryField,
    type_name: &str,
) -> String {
    let mut result = String::new();
...
fn to_apply_insert_row_function(query: &ast::Query) -> String {
    ...
    result.push_str("applyInsertRow : ( String, List Db.Delta.PathSegment ) -> Int -> Decode.Value -> ReturnData -> Result String ReturnData\n");
    result.push_str("applyInsertRow ( field, segments ) index rowJson data =\n");
    result.push_str("    case field of\n");
    ...
    for field in &query.fields {
        if let ast::TopLevelQueryField::Field(query_field) = field {
            ...
            result.push_str(&format!("        \"{}\" ->\n", field_name));
            result.push_str(&format!(
                "            applyInsertRowTo{} segments index rowJson data.{}\n",
                capitalized, field_name
            ));
            result.push_str(&format!(
                "                |> Result.map (\\newList -> {{ data | {} = newList }})\n\n",
    ...
    // Generate type-specific insertRow functions
    for field in &query.fields {
        if let ast::TopLevelQueryField::Field(query_field) = field {
            result.push_str(&generate_insert_row_type_function(query_field));
        }
    }
...
/// Generate the applyInsertRowTo{Type} function for a query field
fn generate_insert_row_type_function(query_field: &ast::QueryField) -> String {
    let mut result = String::new();
    let field_name = ast::get_aliased_name(query_field);
    let capitalized = string::capitalize(&field_name);

    result.push_str(&format!(
        "applyInsertRowTo{} : List Db.Delta.PathSegment -> Int -> Decode.Value -> List {} -> Result String (List {})\n",
        capitalized, capitalized, capitalized
    ));
    result.push_str(&format!(
        "applyInsertRowTo{} segments index rowJson list =\n",
        capitalized
    ));
    result.push_str("    case segments of\n");
    result.push_str("        [] ->\n");
    ...
    result.push_str("                Ok row ->\n");
    result.push_str("                    Ok (Db.Delta.insertAtIndex index row list)\n\n");
    result.push_str("                Err err ->\n");
    result.push_str(
        "                    Err (\"Failed to decode row: \" ++ Decode.errorToString err)\n\n",
    );
    ...
    result.push_str("                Just item ->\n");
    result.push_str(&format!(
        "                    applyInsertRowNested{} restSegments index rowJson item\n",
        capitalized
    ));
    result.push_str("                        |> Result.map (\\newItem -> Db.Delta.setAtIndex idx newItem list)\n\n");
    result.push_str("                Nothing ->\n");
    ...
    // Generate nested insert function
    let nested_fields = collect_nested_list_fields(query_field);

    result.push_str(&format!(
        "applyInsertRowNested{} : List Db.Delta.PathSegment -> Int -> Decode.Value -> {} -> Result String {}\n",
        capitalized, capitalized, capitalized
    ));
    result.push_str(&format!(
        "applyInsertRowNested{} segments index rowJson item =\n",
        capitalized
    ));
    result.push_str("    case segments of\n");
    ...
    if nested_fields.is_empty() {
        result.push_str("        _ ->\n");
        result.push_str("            Err \"No nested fields to insert into\"\n\n\n");
    } else {
        for (nested_name, nested_type) in &nested_fields {
            result.push_str(&format!(
                "        (Db.Delta.Field \"{}\") :: restSegments ->\n",
            ...
            result.push_str(&format!(
                "            applyInsertRowTo{} restSegments index rowJson item.{}\n",
                nested_type, nested_name
            ));
            result.push_str(&format!(
                "                |> Result.map (\\newNested -> {{ item | {} = newNested }})\n\n",
    ...
    for (nested_name, nested_type) in &nested_fields {
        if let Some(nested_field) = find_nested_query_field(query_field, nested_name) {
            result.push_str(&generate_insert_row_nested_type_function(
                nested_field,
                nested_type,
            ));
        }
...
/// Generate insertRow functions for nested types
fn generate_insert_row_nested_type_function(
    query_field: &ast::QueryField,
    type_name: &str,
) -> String {
    let mut result = String::new();
...
fn to_apply_move_row_function(query: &ast::Query) -> String {
    ...
    result.push_str(
        "applyMoveRow : ( String, List Db.Delta.PathSegment ) -> Int -> Int -> ReturnData -> Result String ReturnData\n",
    );
    result.push_str("applyMoveRow ( field, segments ) from to data =\n");
    result.push_str("    case field of\n");
    ...
    for field in &query.fields {
        if let ast::TopLevelQueryField::Field(query_field) = field {
            ...
            result.push_str(&format!("        \"{}\" ->\n", field_name));
            result.push_str(&format!(
                "            applyMoveRowIn{} segments from to data.{}\n",
                capitalized, field_name
            ));
            result.push_str(&format!(
                "                |> Result.map (\\newList -> {{ data | {} = newList }})\n\n",
    ...
    // Generate type-specific moveRow functions
    for field in &query.fields {
        if let ast::TopLevelQueryField::Field(query_field) = field {
            result.push_str(&generate_move_row_type_function(query_field));
        }
    }
...
/// Generate the applyMoveRowIn{Type} function for a query field
fn generate_move_row_type_function(query_field: &ast::QueryField) -> String {
    let mut result = String::new();
    let field_name = ast::get_aliased_name(query_field);
    let capitalized = string::capitalize(&field_name);

    result.push_str(&format!(
        "applyMoveRowIn{} : List Db.Delta.PathSegment -> Int -> Int -> List {} -> Result String (List {})\n",
        capitalized, capitalized, capitalized
    ));
    result.push_str(&format!(
        "applyMoveRowIn{} segments from to list =\n",
        capitalized
    ));
    result.push_str("    case segments of\n");
    result.push_str("        [] ->\n");
    result.push_str("            -- Move at this list level\n");
    result.push_str("            Ok (Db.Delta.moveInList from to list)\n\n");
    result.push_str("        (Db.Delta.Index idx) :: restSegments ->\n");
    result.push_str("            case Db.Delta.getAtIndex idx list of\n");
    result.push_str("                Just item ->\n");
    result.push_str(&format!(
        "                    applyMoveRowNested{} restSegments from to item\n",
        capitalized
    ));
    result.push_str("                        |> Result.map (\\newItem -> Db.Delta.setAtIndex idx newItem list)\n\n");
    result.push_str("                Nothing ->\n");
    ...
    // Generate nested move function
    let nested_fields = collect_nested_list_fields(query_field);

    result.push_str(&format!(
        "applyMoveRowNested{} : List Db.Delta.PathSegment -> Int -> Int -> {} -> Result String {}\n",
        capitalized, capitalized, capitalized
    ));
    result.push_str(&format!(
        "applyMoveRowNested{} segments from to item =\n",
        capitalized
    ));
    result.push_str("    case segments of\n");
    ...
    if nested_fields.is_empty() {
        result.push_str("        _ ->\n");
        result.push_str("            Err \"No nested fields to move in\"\n\n\n");
    } else {
        for (nested_name, nested_type) in &nested_fields {
            result.push_str(&format!(
                "        (Db.Delta.Field \"{}\") :: restSegments ->\n",
            ...
            result.push_str(&format!(
                "            applyMoveRowIn{} restSegments from to item.{}\n",
                nested_type, nested_name
            ));
            result.push_str(&format!(
                "                |> Result.map (\\newNested -> {{ item | {} = newNested }})\n\n",
    ...
    for (nested_name, nested_type) in &nested_fields {
        if let Some(nested_field) = find_nested_query_field(query_field, nested_name) {
            result.push_str(&generate_move_row_nested_type_function(
                nested_field,
                nested_type,
            ));
        }
...
/// Generate moveRow functions for nested types
fn generate_move_row_nested_type_function(
    query_field: &ast::QueryField,
    type_name: &str,
) -> String {
    let mut result = String::new();
...
fn to_apply_remove_row_by_index_function(query: &ast::Query) -> String {
    ...
    result.push_str(
        "applyRemoveRowByIndex : ( String, List Db.Delta.PathSegment ) -> Int -> ReturnData -> Result String ReturnData\n",
    );
    result.push_str("applyRemoveRowByIndex ( field, segments ) index data =\n");
    result.push_str("    case field of\n");
    ...
    for field in &query.fields {
        if let ast::TopLevelQueryField::Field(query_field) = field {
            ...
            result.push_str(&format!("        \"{}\" ->\n", field_name));
            result.push_str(&format!(
                "            applyRemoveRowByIndexFrom{} segments index data.{}\n",
                capitalized, field_name
            ));
            result.push_str(&format!(
                "                |> Result.map (\\newList -> {{ data | {} = newList }})\n\n",
    ...
    // Generate type-specific removeRowByIndex functions
    for field in &query.fields {
        if let ast::TopLevelQueryField::Field(query_field) = field {
            result.push_str(&generate_remove_row_by_index_type_function(query_field));
        }
    }
...
/// Generate the applyRemoveRowByIndexFrom{Type} function for a query field
fn generate_remove_row_by_index_type_function(query_field: &ast::QueryField) -> String {
    let mut result = String::new();
    let field_name = ast::get_aliased_name(query_field);
    let capitalized = string::capitalize(&field_name);

    result.push_str(&format!(
        "applyRemoveRowByIndexFrom{} : List Db.Delta.PathSegment -> Int -> List {} -> Result String (List {})\n",
        capitalized, capitalized, capitalized
    ));
    result.push_str(&format!(
        "applyRemoveRowByIndexFrom{} segments index list =\n",
        capitalized
    ));
    result.push_str("    case segments of\n");
    result.push_str("        [] ->\n");
    result.push_str("            -- Remove at this list level\n");
    result.push_str("            Ok (Db.Delta.removeAtIndex index list)\n\n");
    result.push_str("        (Db.Delta.Index idx) :: restSegments ->\n");
    result.push_str("            case Db.Delta.getAtIndex idx list of\n");
    result.push_str("                Just item ->\n");
    result.push_str(&format!(
        "                    applyRemoveRowByIndexNested{} restSegments index item\n",
        capitalized
    ));
    result.push_str("                        |> Result.map (\\newItem -> Db.Delta.setAtIndex idx newItem list)\n\n");
    result.push_str("                Nothing ->\n");
    ...
    result.push_str(&format!(
        "applyRemoveRowByIndexNested{} : List Db.Delta.PathSegment -> Int -> {} -> Result String {}\n",
        capitalized, capitalized, capitalized
    ));
    result.push_str(&format!(
        "applyRemoveRowByIndexNested{} segments index item =\n",
        capitalized
    ));
    result.push_str("    case segments of\n");
    ...
    if nested_fields.is_empty() {
        result.push_str("        _ ->\n");
        result.push_str("            Err \"No nested fields to remove from\"\n\n\n");
    } else {
        for (nested_name, nested_type) in &nested_fields {
            result.push_str(&format!(
                "        (Db.Delta.Field \"{}\") :: restSegments ->\n",
            ...
            result.push_str(&format!(
                "            applyRemoveRowByIndexFrom{} restSegments index item.{}\n",
                nested_type, nested_name
            ));
            result.push_str(&format!(
                "                |> Result.map (\\newNested -> {{ item | {} = newNested }})\n\n",
    ...
    for (nested_name, nested_type) in &nested_fields {
        if let Some(nested_field) = find_nested_query_field(query_field, nested_name) {
            result.push_str(&generate_remove_row_by_index_nested_type_function(
                nested_field,
                nested_type,
            ));
        }
...
/// Generate removeRowByIndex functions for nested types
fn generate_remove_row_by_index_nested_type_function(
    query_field: &ast::QueryField,
    type_name: &str,
) -> String {
    let mut result = String::new();
...
fn to_delta_helper_functions() -> String {
    // Helper functions are now in Db.Delta module, nothing to generate here
    String::new()
}
